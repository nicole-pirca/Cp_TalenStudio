...
package cp_talenstudio.df_test_0_1;
 ...
 * @status
 */
public class DF_TEST implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "DF_TEST.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(DF_TEST.class);
	...
	private final String jobName = "DF_TEST";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(DF_TEST.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_CP_TALENSTUDIO_DF_TEST = new byte[0];
		static byte[] commonByteArray_CP_TALENSTUDIO_DF_TEST = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_CP_TALENSTUDIO_DF_TEST.length) {
					if (length < 1024 && commonByteArray_CP_TALENSTUDIO_DF_TEST.length == 0) {
						commonByteArray_CP_TALENSTUDIO_DF_TEST = new byte[1024];
					} else {
						commonByteArray_CP_TALENSTUDIO_DF_TEST = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_CP_TALENSTUDIO_DF_TEST, 0, length);
				strReturn = new String(commonByteArray_CP_TALENSTUDIO_DF_TEST, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_CP_TALENSTUDIO_DF_TEST, 0, length);
				strReturn = new String(commonByteArray_CP_TALENSTUDIO_DF_TEST, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_CP_TALENSTUDIO_DF_TEST) {
...
///////////////////////    			
				...
				for (int itRowGenerator_1 = 0; itRowGenerator_1 < nb_max_row_tRowGenerator_1; itRowGenerator_1++) {
					row1.Nombre = randtRowGenerator_1.getRandomNombre();
...
///////////////////////		
					...
					if (row1.Nombre != null) { //
						...
						strBuffer_tLogRow_1.append(String.valueOf(row1.Nombre));
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final DF_TEST DF_TESTClass = new DF_TEST();

		int exitCode = DF_TESTClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'DF_TEST' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		getjobInfo();
		log.info("TalendJob: 'DF_TEST' - Start.");
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_fSFkkBjQEfCKKv3E7CbXdA");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2025-04-14T01:34:31.744923200Z");
		...
		try {
			...
			java.io.InputStream inContext = DF_TEST.class.getClassLoader()
					.getResourceAsStream("cp_talenstudio/df_test_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = DF_TEST.class.getClassLoader()
		...
		log.info("TalendJob: 'DF_TEST' - Started.");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : DF_TEST");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'DF_TEST' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 62233 characters generated by Talend Cloud Data Fabric on the 13 de abril de
 * 2025, 20:34:31 ECT
 ************************************************************************************************/